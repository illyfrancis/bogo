...
app.views.SearchFilters = Backbone.View.extend({

	template: _.template($("#tpl-search-filters").html()),
	...
	appendFilter: function(reportCriteria) {
		...
		app.views.viewManager.filterView(reportCriteria);
	},

	appendFilterSelector: function(reportCriteria) {
		var filterSelector = new app.views.SearchFilterSelector({
	...
	setSearchFilter: function(criteriaName) {
		...
		filterSelector && filterSelector.select();
	},
	...
	changeSearchFilter: function(cid) {
		...
		this.currentReportCriteria = this.collection.getByCid(cid)
		var selectedFilter = app.views.viewManager.filterView(this.currentReportCriteria);
	...
	showAlert: function(message) {
		this.$el.find(".modal-body div.filter-content").prepend(this.templateAlert({message:message}));
	},
