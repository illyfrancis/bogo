...
app.views.AccountFilter = Backbone.View.extend({
	...
	initialize: function() {
		...
		app.data.paginatedAccounts.on("reset", this.renderAccountList, this);
		app.data.paginatedAccounts.on("change:selected", this.filterChanged, this);

		// keep track of account rows
		this._accountRows = [];

		this.paginator = new app.views.AccountPaginator({
			collection: app.data.paginatedAccounts
		});
	},
	...
	filterChanged: function() {
		...
		if (this.model.get("isApplied")) {
			if (!app.data.paginatedAccounts.hasSelection()) {
	...
	updateAccountSelection: function(checked) {
		...
		app.data.paginatedAccounts.selectAll(checked);
	},
	...
	filterAccounts: function() {
		...
		app.data.paginatedAccounts.setFieldFilter(fieldFilters);
	...
	renderAccountList: function() {
		...
		this.$table.empty();
		app.data.paginatedAccounts.each(this.appendAccountRow, this);
		return this;
	},
